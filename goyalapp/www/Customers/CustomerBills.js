			...
			callback: (response) => {
				...
				if (data.length > 0) {
					for (let i = 0; i < data.length; i++) {
						...
						let content = `<tr class='resultcontent'><td class='table-sr'>${i+1}</td><td class='table-sr'>${rowdata.posting_date}</td><td class='table-sr'>${rowdata.customer}</td><td class='table-sr'>${rowdata.customer_name}</td><td class='table-sr'>${rowdata.vehicle_no}</td><td class='table-sr'>${rowdata.total_qty}</td>`;
						$(content).appendTo(result_wrapper);
					}
				} else {
